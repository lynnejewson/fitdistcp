...
# the following functions do nothing at all; I decided to leave them in for now, and possibly remove after converting more modules?
# Extract the results from derivatives and put them into f2
def deriv_copyfdd(temp1, nx, dim):
    return temp1
         
# Extract the results from derivatives and put them into ldd
def deriv_copyldd(temp1, nx, dim):
    ldd = np.sum(temp1, axis=-1) / nx
    return ldd

def deriv_copylddd(temp1, nx, dim):
    lddd = np.sum(temp1, axis=-1) / nx
    return lddd

def deriv_copyld2(temp1, nx, dim):
    return temp1
